...
// Function to store the last active page of the user
function setActivePage() {
    ...
    links.forEach(link => {
        if (link.href.includes(lastActivePage)) {
...
function saveActivePage(event) {
    const clickedPage = event.target.href;
    localStorage.setItem('lastActivePage', clickedPage);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const links = document.querySelectorAll('.navigation a');
    links.forEach(link => {
        link.addEventListener('click', saveActivePage);
    });
});
